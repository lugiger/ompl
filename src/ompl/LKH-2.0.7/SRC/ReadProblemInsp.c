...
static void Read_DIMENSION(char * params, char ** saveptr);
static void Read_DISPLAY_DATA_SECTION(char * params, char ** saveptr);
static void Read_DISPLAY_DATA_TYPE(char * params, char ** saveptr);
static void Read_EDGE_DATA_FORMAT(char * params, char ** saveptr);
static void Read_EDGE_DATA_SECTION(char * params, char ** saveptr);
static void Read_EDGE_WEIGHT_FORMAT(char * params, char ** saveptr);
static void Read_EDGE_WEIGHT_SECTION(char * params, char ** saveptr);
static void Read_EDGE_WEIGHT_TYPE(char * params, char ** saveptr);
static void Read_FIXED_EDGES_SECTION(char * params, char ** saveptr);
static void Read_NAME(char * params, char ** saveptr);
static void Read_NODE_COORD_SECTION(int i, double ** VP);
static void Read_NODE_COORD_TYPE(char * params, char ** saveptr);
static void Read_TOUR_SECTION(FILE ** File);
static void Read_TYPE(char * params, char ** saveptr);
static int TwoDWeightType(void);
static int ThreeDWeightType(void);

void ReadProblemInsp(int d, double ** VP, char * params)
{
    
    int i, K;
    char *Line, *Keyword, *saveptr;
    ...
    C = 0;
    c = 0;
    Line = strtok_r(params, "\n", &saveptr);
    while ((Line)) {
        char *tmpptrStart = 0;
        if (!(Keyword = strtok_r(Line, Delimiters, &tmpptrStart)))
            continue;
        ...
        if (!strcmp(Keyword, "COMMENT"));
            ...
            Read_DIMENSION(Line, &tmpptrStart);
        else if (!strcmp(Keyword, "DISPLAY_DATA_SECTION"))
            Read_DISPLAY_DATA_SECTION(params, &saveptr);
        else if (!strcmp(Keyword, "DISPLAY_DATA_TYPE"))
            Read_DISPLAY_DATA_TYPE(Line, &tmpptrStart);
        else if (!strcmp(Keyword, "EDGE_DATA_FORMAT"))
            Read_EDGE_DATA_FORMAT(Line, &tmpptrStart);
        else if (!strcmp(Keyword, "EDGE_DATA_SECTION"))
            Read_EDGE_DATA_SECTION(params, &saveptr);
        else if (!strcmp(Keyword, "EDGE_WEIGHT_FORMAT"))
            Read_EDGE_WEIGHT_FORMAT(Line, &tmpptrStart);
        else if (!strcmp(Keyword, "EDGE_WEIGHT_SECTION"))
            Read_EDGE_WEIGHT_SECTION(params, &saveptr);
        else if (!strcmp(Keyword, "EDGE_WEIGHT_TYPE"))
            Read_EDGE_WEIGHT_TYPE(Line, &tmpptrStart);
        else if (!strcmp(Keyword, "EOF"))
            break;
        else if (!strcmp(Keyword, "FIXED_EDGES_SECTION"))
            Read_FIXED_EDGES_SECTION(params, &saveptr);
        else if (!strcmp(Keyword, "NAME"))
            Read_NAME(Line, &tmpptrStart);
        else if (!strcmp(Keyword, "NODE_COORD_SECTION"))
            Read_NODE_COORD_SECTION(d, VP);
        else if (!strcmp(Keyword, "NODE_COORD_TYPE"))
            Read_NODE_COORD_TYPE(Line, &tmpptrStart);
        else if (!strcmp(Keyword, "TOUR_SECTION"))
            eprintf("Option TOUR_SECTION currently not fully supported");//Read_TOUR_SECTION(&ProblemFile);
        else if (!strcmp(Keyword, "TYPE"))
            Read_TYPE(Line, &tmpptrStart);
        else
        ...
        do {
            ...
            if (ProblemType != HPP || Ni->Id < Dimension)
                for (Nj = FirstNode; Nj != Ni; Nj = Nj->Suc)
                    Ni->C[Nj->Id] = Fixed(Ni, Nj) ? 0 : Distance(Ni, Nj); //TODO: precalculate candidate set!
            else
                for (Nj = FirstNode; Nj != Ni; Nj = Nj->Suc)
                    Ni->C[Nj->Id] = 0;
        }
...
static void Read_NAME(char * params, char ** saveptr)
{
    if (!(Name = Copy(strtok_r(0, Delimiters, saveptr))))
...
static void Read_DISPLAY_DATA_SECTION(char * params, char ** saveptr)
{
    ...
    for (i = 1; i <= Dimension; i++) {
        char * tmp = strtok_r(0,Delimiters,saveptr);
        if (!fscanintC(tmp, &Id))
        ...
        if (N->V == 1)
            eprintf("(DIPLAY_DATA_SECTION) Node number occours twice: %d",
                    N->Id);
        N->V = 1;
        tmp = strtok_r(0,Delimiters,saveptr);
        if ((N->X = atof(tmp)) && tmp[0]!='0')
            eprintf("Missing X-coordinate in DIPLAY_DATA_SECTION");
        tmp = strtok_r(0,Delimiters,saveptr);
        if ((N->Y = atof(tmp)) && tmp[0]!='0')
...
static void Read_DISPLAY_DATA_TYPE(char * params, char ** saveptr)
{
    ...
    if (!(DisplayDataType = Copy(strtok_r(0, Delimiters, saveptr))))
...
static void Read_EDGE_DATA_FORMAT(char * params, char ** saveptr)
{
    ...
    if (!(EdgeDataFormat = Copy(strtok_r(0, Delimiters, saveptr))))
...
static void Read_EDGE_WEIGHT_FORMAT(char * params, char ** saveptr)
{
    ...
    if (!(EdgeWeightFormat = Copy(strtok_r(0, Delimiters, saveptr))))
...
static void Read_EDGE_WEIGHT_TYPE(char * params, char ** saveptr)
{
    ...
    if (!(EdgeWeightType = Copy(strtok_r(0, Delimiters,saveptr))))
...
static void Read_NODE_COORD_SECTION(int d, double ** VP)
{
    Node *N;
    int Id, i;
    ...
    for (i = 0; i < Dimension; i++) {
        ...
        if (N->V == 1)
            eprintf("(NODE_COORD_SECTION) Node number occours twice: %d",
                    N->Id);
        N->V = 1;
        N->X = VP[i%DimensionSaved][0];
        N->Y = VP[i%DimensionSaved][1];
        if (CoordType == THREED_COORDS)
          N->Z = VP[i%DimensionSaved][2];
    }
    N = FirstNode;
    do
...
static void Read_NODE_COORD_TYPE(char * params, char ** saveptr)
{
    ...
    if (!(NodeCoordType = Copy(strtok_r(0, Delimiters,saveptr))))
...
static void Read_TYPE(char * params, char ** saveptr)
{
    ...
    if (!(Type = Copy(strtok_r(0, Delimiters,saveptr))))
        eprintf("TYPE: string expected, %s, %s", *saveptr, Type);
